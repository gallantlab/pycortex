...
In the WebGL viewer, tractogram controls live in their own panel under the dataset box, with one entry per tractogram: a visibility checkbox, the name and a collapse toggle in the header, and an opacity slider in the (optionally collapsed) body. A tractogram with at least one group also gets a "bundles" section below that slider, with "all"/"none" buttons plus one checkbox per group, listed alphabetically rather than in the order they were given and each labeled with its streamline count; any streamline in no group is covered by an additional ``(ungrouped)`` checkbox. A streamline renders while it belongs to at least one checked group. In a dataset holding several subjects, a tractogram is shown only while a dataview of its own subject is the active one, since its coordinates mean nothing against another brain. From Python, toggle a group directly on the live viewer handle, e.g. ``handle.tracts.af.setGroupVisible("CST_L", False)``.
