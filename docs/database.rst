...
Two of them hold more than one value per vertex and so cannot be returned as a ``Vertex`` object. ``bumpy_flatmap.npz`` holds the relaxed bumpy flatmap (see :class:`cortex.polyutils.FlatSlab`) as a three-component offset per vertex, under the keys ``bump_left`` and ``bump_right``, and ``equivolume_areas.npz`` holds the white matter and pial vertex areas the webgl viewer's equivolume depth sampling needs, under ``wm_left``, ``wm_right``, ``pia_left`` and ``pia_right``. For these, ``cortex.db.get_surfinfo`` hands back the ``.npz`` file itself rather than a ``Vertex``; remember to close it.
