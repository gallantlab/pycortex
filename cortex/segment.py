...
def init_subject(subject, filenames, do_import_subject=False, **kwargs):
    ...
    if "run_all" in kwargs:
        warnings.warn(
            "`run_all` is deprecated - please use do_import_subject keyword arg instead!"
        )
...
def cut_surface(
    ...
    **kwargs,
    ...
    if not os.path.exists(fname) or recache:
        ...
        blender.fs_cut_init(
            fname, fs_subject, hemi, freesurfer_subject_dir, blender_path=blender_path
        )
    ...
    if not os.path.exists(base_patch_path) or recache:
        ...
        blender.write_volume_patch(
            fname, base_patch_path, hemi, blender_path=blender_path
        )
    """
    ...
    if flatten_with == "blender":
        ...
        print(
            "Generating flat patch via blender method %s to %s..."
            % (method, flat_patch_path)
        )
        done = blender.write_flat_patch(
            fname, flat_patch_path, hemi, method=method, blender_path=blender_path
        )
...
def flatten_slim(
    ...
    if slim_path == "None":
        ...
        raise ValueError(
            "Please download SLIM ({slim_url}) and set the path to it in the `slim` field\n"
            "in the `[dependency_paths]` section of your config file ({usercfg}) \n"
            "if you wish to use slim!".format(
                slim_url=slim_url, usercfg=options.usercfg
            )
        )
    if do_flatten is None:
        resp = input(
            "Flattening with SLIM will take a few mins. Continue? (type y or n and press return)"
        )
    ...
    if freesurfer_subject_dir is None:
        freesurfer_subject_dir = os.environ["SUBJECTS_DIR"]
    patchpath = freesurfer.get_paths(
    ...
    pts, polys, _ = freesurfer.get_surf(
        subject,
        hemi,
        "patch",
        patch=patch,
...
def show_surface(
    ...
    fpath = freesurfer.get_paths(
        subject, hemi, input_type, freesurfer_subject_dir=freesurfer_subject_dir
    )

    if "obj" not in fpath:
        pts, polys, curv = freesurfer.get_surf(
            subject,
            hemi,
            surface_type,
            patch=patch,
...
def fix_wm(subject):
    ...
    """
    warnings.warn(
        "Deprecated! We recommend using edit_segmentation() and rerun_recon() instead of fix_wm() and fix_pia()."
    )
    status = _cycle_surf(subject, "smoothwm")
    cmd = (
        "tkmedit {subj} wm.mgz lh.smoothwm -aux brainmask.mgz -aux-surface rh.smoothwm"
    )
...
def fix_pia(subject):
    ...
    cmd = (
        "tkmedit {subj} brainmask.mgz lh.smoothwm -aux T1.mgz -aux-surface rh.smoothwm"
    )
    sp.call(shlex.split(cmd.format(subj=subject)))
    status.value = 0
