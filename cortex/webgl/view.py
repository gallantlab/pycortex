...
def make_static(
    ...
    **kwargs
    ...
    metadata = package.metadata(
        fmt="data/{name}_{frame}.png",
        tract_fmt="tracts/{name}_{buf}.bin",
        submap=submap,
    )
    ...
    # Write out the raw streamline buffers of any tractogram. The file names
    # use the transport id rather than the dataset key, which is an arbitrary
    # string -- a key of "../escape" would otherwise write outside `outpath`
    # (see cortex.webgl.data._tract_id).
    if package.tracts:
        tractpath = os.path.join(outpath, "tracts")
        os.makedirs(tractpath, exist_ok=True)
        for name, bufs in package.tracts.items():
            ident = package.tract_ids[name]
            for buf, contents in bufs.items():
                fname = os.path.join(tractpath, "%s_%s.bin" % (ident, buf))
...
def show(
    ...
    **kwargs
    ...
    metadata = package.metadata()
    images = package.images
    # Raw streamline buffers, served by TractHandler below, keyed by the
    # transport id their urls were built from (not by the dataset key, which
    # may hold any character at all -- see cortex.webgl.data._tract_id).
    tracts = dict(
        (package.tract_ids[name], bufs) for name, bufs in package.tracts.items()
    )
    subjects = list(package.subjects)
    ...
    class TractHandler(web.RequestHandler):
        """Serve the raw streamline buffers of a Tractogram.

        Mirrors DataHandler, for the "/tract/{id}/{buf}/" urls generated by
        `cortex.webgl.data.Package.tract_names`, where `id` is a tractogram's
        transport id (`cortex.webgl.data._tract_id`). `buf` is one of "points"
        (float32 N x 3), "offsets" (uint32 M + 1), "colors" (uint8 N x 3) or
        "groups" (uint32, concatenated group streamline indices).
        """
        def get(self, path: str):
            path = path.strip("/")
            try:
                tractname, buf = path.rsplit('/', 1)
            except ValueError:
                self.set_status(404)
                self.write_error(404)
                return

            if tractname in tracts and buf in tracts[tractname]:
                contents = tracts[tractname][buf]
                self.set_header("Content-Type", "application/octet-stream")
    ...
    class JSMixer(serve.JSProxy[P]):
        ...
        def addData(self, **kwargs):
            ...
            # require_brains=False: the viewer is already up, so pushing only
            # a Tractogram is fine (the javascript side just adds it to the
            # scene and keeps the currently displayed dataview).
            new_package = Package(new_data, require_brains=False)
            ...
            # Serve the streamline buffers of any newly added tractogram, and
            # remember them so that a page reload still shows them.
            tracts.update(
                (new_package.tract_ids[name], bufs)
                for name, bufs in new_package.tracts.items()
            )
            metadata["tracts"].update(new_metadata["tracts"])
