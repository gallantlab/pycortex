...
def make_static(
    ...
    **kwargs
    ...
    # Write out the raw streamline buffers of any tractogram
    if package.tracts:
        ...
        for name, bufs in package.tracts.items():
            for buf, contents in bufs.items():
                fname = os.path.join(tractpath, "%s_%s.bin" % (name, buf))
...
def show(
    ...
    **kwargs
    ...
    metadata = package.metadata()
    images = package.images
    # Raw streamline buffers, served by TractHandler below.
    tracts = package.tracts
    subjects = list(package.subjects)
    ...
    class TractHandler(web.RequestHandler):
        ...
        Mirrors DataHandler, for the "/tract/{name}/{buf}/" urls generated by
        `cortex.webgl.data.Package.tract_names`. `buf` is one of "points"
    ...
    class JSMixer(serve.JSProxy[P]):
        ...
        def addData(self, **kwargs):
            ...
            tracts.update(new_package.tracts)
