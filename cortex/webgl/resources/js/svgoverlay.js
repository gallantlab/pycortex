...
var svgoverlay = (function(module) {
    ...
    module.SVGOverlay = function(svgpath, posdata, callback) {
    ...
    module.Labels = function(labels, posdata) {
        ...
        // var defs = this.doc.createElementNS(svgns, "defs");
        // var filter = labels.parentNode.parentNode.getElementById("dropshadow");
        ...
        this.num = 0;
        this.indices = {left:{idx:[],panel:[]}, right:{idx:[],panel:[]}}
        ...
        for (var i = 0; i < labels.length; i++) {
            ...
            if (idx < leftlen) {
                this.indices.left.idx.push(this.posdata.left.map[idx]);
                this.indices.left.panel.push(this.labels[name].idx);
            } else {
                idx -= leftlen;
                this.indices.right.idx.push(this.posdata.right.map[idx]);
    ...
    module.Labels.prototype._make_object = function() {
        ...
        var position = new Float32Array(this.indices.left.idx.length*3);
        for (var i = 0; i < this.indices.left.idx.length; i++) {
            var idx = this.indices.left.idx[i];
            var vert = mriview.get_position(this.posdata.left, 0, idx);
            position[i*3+0] = vert.pos.x;
            position[i*3+1] = vert.pos.y;
            position[i*3+2] = vert.pos.z;
        }
        var offset = new Float32Array(this.indices.left.idx.length*2);
        this.geometry.left.addAttribute("position", new THREE.BufferAttribute(position, 3));

        var position = new Float32Array(this.indices.right.idx.length*3);
        for (var i = 0; i < this.indices.right.idx.length; i++) {
            var idx = this.indices.right.idx[i];
            var vert = mriview.get_position(this.posdata.right, 0, idx);
            position[i*3+0] = vert.pos.x;
            position[i*3+1] = vert.pos.y;
            position[i*3+2] = vert.pos.z;
        }
        var offset = new Float32Array(this.indices.right.idx.length*2);
        this.geometry.right.addAttribute("position", new THREE.BufferAttribute(position, 3));

        this.meshes = {};
        var shader = new THREE.PointCloudMaterial({size:20, color:'red'});
        this.meshes.left = new THREE.PointCloud(this.geometry.left, shader);
        this.meshes.right = new THREE.PointCloud(this.geometry.right, shader);
    }
    ...
    module.Labels.prototype.update = function() {
        ...
        this.svg.setAttribute("width", nwide * (this.size[0] + 2*padding));
        this.svg.setAttribute("height", ntall * (this.size[1] + 2*padding));

        for (var name in this.labels) {
            var i = this.labels[name].idx;
            var x = Math.floor(i / ntall);
            var y = i % ntall;
    ...
    module.Labels.prototype.setMix = function(mix, pos, norms) {
