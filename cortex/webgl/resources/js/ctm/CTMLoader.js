        ...
        if ( xhr.readyState === 4 ) {
            ...
            if ( xhr.status === 200 || xhr.status === 0 ) {
                ...
                if ( parameters.useWorker ) {
                    ...
                    worker.postMessage( { "data": binaryData.buffer, "offsets": offsets } );
...
THREE.CTMLoader.prototype.createModelBuffers = function ( file, callback ) {
    ...
    var Model = function ( ) {
        ...
        if ( reorderVertices ) {
            ...
            // create duplicate vertices and update sprawled faces
            var indexMap2 = {},
            ...
            var newNormals, newUvs = {}, newAttrs = {};

            if ( vertexNormalArray ) newNormals = new Float32Array( 3*vertexCounter );

            for (var i in vertexUvArrays) {
                newUvs[i] = new Float32Array( 2*vertexCounter );
            }
            for (var i in vertexAttrArrays) {
                newAttrs[i] = new Float32Array( 4*vertexCounter );
            }
            ...
            vertexUvArrays = newUvs;
            vertexAttrArrays = newAttrs;

            scope.indexMap = indexMap;
            scope.reverseIndexMap = reverseIndexMap;
        }
        ...
        // var e = Date.now();
        // console.log( "Vertex reordering time: " + (e-s) + " ms" );
