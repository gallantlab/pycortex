...
def curvature(outfile, subject, smooth=20, **kwargs):
    curvs = []
    for pts, polys in surfs.get_surf(subject, "fiducial"):
...
def distortion(outfile, subject, type='areal', smooth=20):
    ...
    distortions = []
    for hem in ["lh", "rh"]:
        fidvert, fidtri = surfs.get_surf(subject, "fiducial", hem)
        flatvert, flattri = surfs.get_surf(subject, "flat", hem)
...
def thickness(outfile, subject):
    pl, pr = surfs.get_surf(subject, "pia")
    wl, wr = surfs.get_surf(subject, "wm")
...
def tissots_indicatrix(outfile, sub, radius=10, spacing=50, maxfails=100): 
    ...
    allcenters = []
    for hem in ["lh", "rh"]:
        fidpts, fidpolys = surfs.get_surf(sub, "fiducial", hem)
...
def flat_border(outfile, subject):
    flatpts, flatpolys = surfs.get_surf(subject, "flat", merge=True, nudge=True)
    flatpolyset = set(map(tuple, flatpolys))
    
    fidpts, fidpolys = surfs.get_surf(subject, "fiducial", merge=True, nudge=True)
