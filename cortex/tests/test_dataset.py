...
def test_braindata():
	...
	mask = surfs.get_mask(subj, xfmname, "thick")
...
def test_findmask():
	...
	mask = surfs.get_mask(subj, xfmname, "thin")
...
def test_pack():
	...
	ds = cortex.openFile(tf.name)
	pts, polys = cortex.surfs.get_surf(subj, "fiducial", "lh")
	dpts, dpolys = ds.get_surf(subj, "fiducial", "lh")
	assert np.allclose(pts, dpts)

	rois = cortex.surfs.get_overlay(subj, "rois")
	assert rois.rois.keys() == ds.get_overlay(subj, "rois").rois.keys()

	xfm = cortex.surfs.get_xfm(subj, xfmname)
