...
`Tractogram` (`tractogram.py`) is the odd one out: a `Dataview` that is not braindata at all, holding streamlines as a flat `(N, 3)` point array plus an `(M+1,)` offset table (the TRX convention), loaded with `Tractogram.from_trx` through the optional `trx-python` extra (lazy import, so pycortex still works without it: `trx-python` needs Python 3.11, which is why `run_tests.yml` floors its matrix there while `install_from_wheel.yml` still covers 3.10). It is viewer-only: quickflat has no path for it (passing one to `quickshow` raises in `make_flatmap_image`, it is not silently skipped), `_write_hdf` raises, and `Package` refuses to build a viewer from a tractogram alone.
...
- Streamlines (`Tractogram`) bypass the mosaic/CTM path entirely: `Package` collects them into raw little-endian buffers served by `view.py:TractHandler` under `/tract/` (or written to `tracts/*.bin` by `make_static`), and `resources/js/tractogram.js` builds one `THREE.Line`/`LinePieces` per tractogram plus the `#tracts` DOM panel under the dataset box. Two subtleties: `svgoverlay.js`'s depth pass swaps in a `scene.overrideMaterial` that the line geometry cannot satisfy, so tract objects opt out via `userData.skipOverrideMaterial`; and r69 walks the transparent list backwards, so a translucent tract needs a *large* `renderDepth` to stay behind the surface.
...
- `filestore/db/S1/overlays.svg` is committed in the form `svgoverlay.get_overlay` writes it (lxml pretty-printed, with all default layers including `display`), so tests and examples should leave it unchanged. If `git status` shows it modified, something changed its content; do not commit that unless it is intentional.
